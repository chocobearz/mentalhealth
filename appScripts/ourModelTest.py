...
features = posts.drop(['rating', 'id', 'type'], axis = 1)
rating = posts['rating']
...
thing = featureTest['text']
featureTrain = featureTrain.drop('text', axis = 1)
featureTest = featureTest.drop('text', axis = 1)
...
for i in range(10):
  pred(3, thing.iloc[i], weights, intercept)
  print(featureTest.iloc[i,])
  print(predictions[i])
